...
def startMatch():
    ...
    input_ = "cd ~ && cd " + DIR_MONITOR + " && rcssmonitor --auto-reconnect-mode on"
    ...
    for strategy in strategies:
        ...
        for i in range(10):
            ...
            results.append(tuple([int(scores[0]), int(scores[1])]))
            sleep(5)
        # Fim da execução da estratégia
        fim = time.time()
        saveScore(f'{DIR_LOG}/resultados.csv', results)
...
def saveScore(path_file, results):
    ...
    with open(path_file, mode) as f:
        ...
        if mode == 'w': 
            csv_writer.writerow(['our score', 'opponent score'])
