...
namespace
{
    ...
    struct DeflectingEvaluator
    {
                          ...
                          TackleGenerator::TackleResult &result) const
        {
            ...
            if (result.ball_vel_.x < 0.0 && wm.ball().pos().x > ServerParam::i().ourTeamGoalLineX() + 0.5)
            {
 ...
 */
void TackleGenerator::TackleResult::clear()
{
    tackle_angle_ = 0.0;
 ...
 */
void TackleGenerator::generate(const WorldModel &wm)
{
    ...
    if (wm.self().tackleProbability() < 0.001 && wm.self().foulProbability() < 0.001)
    {
        // dlog.addText( Logger::CLEAR,
 ...
 */
void TackleGenerator::calculate(const WorldModel &wm)
{
    ...
    const double tackle_rate = SP.tacklePowerRate() * (1.0 - 0.5 * ball_rel_angle.abs() / 180.0);
    ...
    for (int a = 0; a < ANGLE_DIVS; ++a)
    {
        ...
        double eff_power = (SP.maxBackTacklePower() + (SP.maxTacklePower() - SP.maxBackTacklePower()) * (1.0 - (dir.abs() / 180.0)));
...
#ifdef ASSUME_OPPONENT_KICK
        if (shootable_area && wm.existKickableOpponent())
        {
            accel += shoot_accel;
            double d = accel.r();
                          ...
                          const TackleResult &result)
{
    ...
    //
    if (ball_end_point.x > SP.pitchHalfLength() && wm.ball().pos().dist2(SP.theirTeamGoalPos()) < std::pow(20.0, 2))
    {
        ...
        if (intersect.isValid() && intersect.absY() < SP.goalHalfWidth())
        {
            double shoot_score = 1000000.0;
            double speed_rate = 1.0 - std::exp(-std::pow(ball_speed, 2) / (2.0 * std::pow(SP.ballSpeedMax() * 0.5, 2)));
            double y_rate = std::exp(-std::pow(intersect.absY(), 2) / (2.0 * std::pow(SP.goalWidth(), 2)));
    ...
    if (ball_end_point.x < -SP.pitchHalfLength())
    {
        ...
        if (intersect.isValid() && intersect.absY() < SP.goalHalfWidth() + 1.0)
        {
            ...
            double y_penalty = (-10000.0 * std::exp(-std::pow(intersect.absY() - SP.goalHalfWidth(), 2) / (2.0 * std::pow(SP.goalHalfWidth(), 2))));
            double speed_bonus = (+10000.0 * std::exp(-std::pow(ball_speed, 2) / (2.0 * std::pow(SP.ballSpeedMax() * 0.5, 2))));
    ...
    double our_goal_angle_rate = 1.0 - std::exp(-std::pow(our_goal_angle_diff, 2) / (2.0 * std::pow(40.0, 2)));

    double y_rate = (final_point.absY() > SP.pitchHalfWidth() - 0.1
                         ? 1.0
                         : std::exp(-std::pow(final_point.absY() - SP.pitchHalfWidth(), 2) / (2.0 * std::pow(SP.pitchHalfWidth() * 0.7, 2))));
    double opp_rate = 1.0 - std::exp(-std::pow((double)opponent_reach_step, 2) / (2.0 * std::pow(30.0, 2)));
    ...
    double x_val = (final_point.x > SP.pitchHalfLength()
                        ? 1.0
                        : std::exp(-std::pow(final_point.x - SP.pitchHalfLength(), 2) / (2.0 * std::pow(SP.pitchLength() * 0.4, 2))));
    double y_val = (final_point.absY() > SP.pitchHalfWidth()
                        ? 1.0
                        : std::exp(-std::pow(final_point.absY() - SP.pitchHalfWidth(), 2) / (2.0 * std::pow(SP.pitchHalfWidth() * 0.7, 2))));

    double opp_goal_dist = SP.theirTeamGoalPos().dist(final_point);
    double opp_goal_dist_val = std::exp(-std::pow(opp_goal_dist, 2) / (2.0 * std::pow(20.0, 2)));

    double our_goal_dist = SP.ourTeamGoalPos().dist(final_point);
    double our_goal_dist_rate = 1.0 - std::exp(-std::pow(our_goal_dist, 2) / (2.0 * std::pow(20.0, 2)));
 ...
 */
int TackleGenerator::predictOpponentsReachStep(const WorldModel &wm,
                                               ...
                                               const AngleDeg &ball_move_angle)
{
    ...
    const Vector2D ball_end_point = inertia_final_point(first_ball_pos,
                                                        first_ball_vel,
                                                        SP.ballDecay());
    if (ball_end_point.absX() > SP.pitchHalfLength() || ball_end_point.absY() > SP.pitchHalfWidth())
    {
        Rect2D pitch = Rect2D::from_center(0.0, 0.0, SP.pitchLength(), SP.pitchWidth());
 ...
 */
int TackleGenerator::predictOpponentReachStep(const AbstractPlayerObject *opponent,
                                              const Vector2D &first_ball_pos,
                                              const Vector2D &first_ball_vel,
                                              const AngleDeg &ball_move_angle,
                                              const int max_cycle)
{
    ...
    for (int cycle = min_cycle; cycle < max_cycle; ++cycle)
    {
        ...
        if (ball_pos.absX() > SP.pitchHalfLength() || ball_pos.absY() > SP.pitchHalfWidth())
        {
