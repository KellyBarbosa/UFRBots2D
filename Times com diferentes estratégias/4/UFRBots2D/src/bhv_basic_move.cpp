...
Bhv_BasicMove::execute( PlayerAgent * agent )
{
	...
	double Px=-53.0; 
	if(agent->world().ball().pos().x>-46) Px=Px-51; //exemplo:-45.PX=-53-51=-104
	...
	{ 
		...
		{ 
			...
			// falta fechar dois if, agent mais proximo e x do agent 
				...
				Bhv_BasicMove().ExecutaTackle(agent); 
				if(agent->world().self().isWithin(Linha)){ // se estiver dentro da linha
					if(!Body_GoToPoint( agent->world().ball().pos(), dist_thr2, dash_power ).execute( agent )){// ira para posica do inimigo; 
						agent->setNeckAction( new Neck_OffensiveInterceptNeck() ); Bhv_BasicMove().ExecutaTackle(agent); 
					}else{ 
						agent->setNeckAction( new Neck_OffensiveInterceptNeck() ); Bhv_BasicMove().ExecutaTackle(agent); 
					}
				}else{ // se nao estiver dentro da linha 
					if(!Body_GoToPoint( inter, dist_thr, dash_power ).execute( agent )){// ira para posica do inimigo; agent->setNeckAction( new Neck_OffensiveInterceptNeck() );
						Bhv_BasicMove().ExecutaTackle(agent);
					}else{ 
						agent->setNeckAction( new Neck_OffensiveInterceptNeck() ); Bhv_BasicMove().ExecutaTackle(agent); 
					}
		...
		}else{// se o x do agent for maior que da bola 
			Bhv_BasicMove().ExecutaTackle(agent); 
	...
	if ( ! Body_GoToPoint( target_point, dist_thr3, dash_power
	                      ).execute( agent ) )
	{
...
bool
Bhv_BasicMove::ExecutaTackle(PlayerAgent * agent){
